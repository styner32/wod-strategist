...
## 2024-03-24 - Path Traversal Risk in Custom File Name Generation

**Vulnerability:** A path traversal and potential collision vulnerability was present in `api/internal/worker/handler.go` where `filepath.Join` and user input were manually concatenated to construct a temporary local file name (`localFilePath := filepath.Join("/tmp", fmt.Sprintf("%s_%s", strings.ReplaceAll(safeSessionID, ".", "_"), filepath.Base(p.FilePath)))`). While some sanitization was present, manually constructing temp file paths from unpredictable payloads is error-prone and can lead to unintended directory traversal or file overwrites across concurrent processes.
**Learning:** Constructing temporary file paths from user input or external payloads using `filepath.Join`, even with basic sanitization functions like `filepath.Base()`, introduces unnecessary complexity and potential vulnerabilities. `os.CreateTemp` is the standard, safe way to handle temporary file creation in Go.
**Prevention:** Replace manual temporary file path construction (`filepath.Join("/tmp", ...)`) with `os.CreateTemp("/tmp", "video-*.mp4")`. This guarantees a secure, unique file is created within the intended directory and avoids any risk of path traversal or collisions between concurrent workers.
