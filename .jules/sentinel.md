...
## 2024-05-24 - Information Exposure in Background Tasks
**Vulnerability:** Raw internal errors (`err.Error()`) from external services (like Google Cloud Storage or Gemini API) were being stored directly in user-facing database fields (`Output` of `AnalysisResult` and `ChunkAnalysisResult`), which could expose sensitive configuration or infrastructural details (CWE-209).
**Learning:** In a decoupled architecture using background workers and a database for task status, it is critical to sanitize error messages before writing them to the database just as strictly as one would for synchronous API responses.
**Prevention:** Always use generic error messages for client-facing database fields and ensure raw errors are logged securely server-side instead.
