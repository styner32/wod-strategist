...
## 2025-05-18 - Prevent Information Exposure via Error Messages in Database

**Vulnerability:** In `api/internal/worker/handler.go`, when a video analysis or chunk analysis failed, the raw internal error string (`err.Error()`) was saved directly into the `Output` field of the `db.AnalysisResult` and `db.ChunkAnalysisResult` models. Since these fields are user-facing (retrieved by the client application), storing raw errors could lead to Information Exposure (CWE-209), potentially leaking sensitive system paths, configuration details, or internal workings to the user.
**Learning:** Error details meant for developers and debugging should be logged securely on the server-side, but never directly exposed to users. User-facing outputs must be generic and safe.
**Prevention:** Always use safe, generic error messages (e.g., "An error occurred during video analysis.") when saving error state to database fields that are returned to the client. Keep actual error details restricted to server logs (e.g., `logger.Log.Error(...)`).
