...
## 2024-04-05 - Fix Path Traversal Vulnerability in SessionID Sanitization
**Vulnerability:** The sanitization of `SessionID` in the background workers uses `strings.ContainsRune(safeSessionID, filepath.Separator)` to check for path traversals *after* calling `filepath.Base()`. `filepath.Base()` inherently removes all path separators, rendering the check ineffective and silently truncating the malicious path instead of rejecting it.
**Learning:** `filepath.Base()` strips out all directory path information, including any separators (like `/` or `\`). By the time we check if the path contains separators, they are already gone. The check must happen on the raw input *before* stripping the base path, or it fails to properly reject potentially malicious input.
**Prevention:** Always validate external identifiers (like `SessionID` or `FilePath`) against path separators *before* applying path manipulation functions like `filepath.Base()` or `filepath.Clean()`.
