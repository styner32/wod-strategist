...
## 2024-03-29 - [Path Traversal in Chunk Merge Endpoint]
**Vulnerability:** A path traversal vulnerability existed where user-supplied SessionIDs could be passed directly into file path generation inside `MergeChunks` and other handler tasks when combining chunks without being properly sanitized using `filepath.Base`.
**Learning:** Even though `SessionID` went through basic `sanitizeIdentifier`, it did not strip out potentially dangerous path components like directory traversal structures. The `MergeChunks` handler passed the `SessionID` directly into worker logic, which creates temporary directories and builds file paths in `/tmp`. If it is not fully sanitized (via `filepath.Base`), it can be exploited.
**Prevention:** Always use `filepath.Base()` when using `SessionID` (and similar identifiers) as part of a file system path, particularly inside worker functions or controllers where files are written to the local disk.
