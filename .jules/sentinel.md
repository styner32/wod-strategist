...
## 2025-05-15 - Prevent Information Exposure via Database Fields

**Vulnerability:** A CWE-209 Information Exposure vulnerability existed in `api/internal/worker/handler.go` where `err.Error()` was directly stored in the `Output` field of the `AnalysisResult` and `ChunkAnalysisResult` database models upon failure. This exposed internal error strings (such as stack traces, file paths, or third-party service details) directly to the end user who queried their analysis results.
**Learning:** Raw internal error details should never be passed down to the client or stored in user-facing database fields. While detailed logging is crucial for debugging on the server side, public-facing errors must be generic and devoid of internal implementation specifics to prevent attackers from gaining insights into the backend architecture.
**Prevention:** Always use generic error messages for client-facing outputs or database records accessed by users (e.g., "Analysis failed due to an internal error."). Log the actual error strictly server-side using a structured logger.
