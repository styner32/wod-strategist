...
## 2026-06-15 - Path Traversal bypass with filepath.Base(".")
**Vulnerability:** The `validateSessionID` function used `sessionID != filepath.Base(sessionID)` to prevent path traversal, but this fails to restrict the exact strings `"."` and `".."`. Since `filepath.Base(".")` returns `"."` and `filepath.Base("..")` returns `".."`, the equality check passes and the malicious session ID is permitted.
**Learning:** `filepath.Base` does not sanitize away `.` and `..` when they are the only elements in a path string. Trusting inequality checks like `input != filepath.Base(input)` without explicitly handling `.` and `..` leaves a gap for directory traversal.
**Prevention:** Always explicitly block exact matches for `.` and `..` alongside using `filepath.Base` inequality or path separator checks to thoroughly block path traversal vectors.
