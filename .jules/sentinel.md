...
## 2025-07-18 - Path Traversal bypass via filepath.Base(".") returning "."
**Vulnerability:** The `validateSessionID` function checked for path traversals by ensuring `sessionID == filepath.Base(sessionID)`. However, passing "." as the input makes `filepath.Base(".")` return "." which matches the input string exactly. This caused the inequality check to be bypassed, allowing payloads like "." or ".." to potentially be accepted as valid session IDs.
**Learning:** `filepath.Base` retains the logical meaning of current and parent directories ("." and ".."). An inequality check against the original input will not catch inputs that are natively just "." or "..".
**Prevention:** In functions guarding against path traversals, explicitly verify that the result of `filepath.Base()` is not "." or "..", nor an empty string, on top of any inequality checks.
