...
## 2026-07-20 - [CRITICAL] Fix API Key Authentication Bypass on Login/Signup Routes
**Vulnerability:** The API Key authentication middleware incorrectly allowed requests to bypass validation if a dummy `jwt` cookie was present. This logic was meant to allow valid web SPA sessions to skip API key checks for protected endpoints since the SPA relies on cookies instead of API keys. However, unauthenticated routes positioned between the API Key middleware and the Authentication middleware (like `/auth/login` and `/auth/signup`) were also skipped, effectively allowing unauthorized requests to bypass rate limiting or any API key requirement for authentication operations.
**Learning:** Middleware skip conditions (e.g., bypassing due to the presence of an unverified credential like a cookie) must explicitly exclude routes that lack downstream validation for those credentials. An unverified cookie should not grant access to endpoints that do not subsequently verify that cookie.
**Prevention:** Avoid blanket middleware bypasses based on unverified client inputs. If a bypass is necessary for specific clients (like SPAs using cookies), explicitly enumerate the allowed routes or exclude specific paths (e.g., authentication endpoints) from the bypass logic to ensure defense in depth.
