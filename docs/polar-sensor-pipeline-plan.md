...
> **후속 분석 설계 (2026-09-07, 미구현):** [단일 테이블 센서 분석·운동 부하 설계](polar-sensor-analysis-design.md)가 업로드 요청 발급, 완료 통지, 재시도, 서버 분석 및 운동 부하 가이드의 최신 계약이다. 아래 문서는 BLE 수집과 원본 기록의 배경이며, 새 분석 기능의 업로드 경로·상태 관리는 후속 설계를 따른다.
...
- **캡처시계 vs 미디어시계 분리 (2026-09-07 교정):**
  ...
  - 두 시계의 변환은 공용 변환기(`api/internal/timeline.CaptureToMedia`)를 통한 **청크별 구간 선형 매핑**으로만 가능하다.
    - 해당 캡처 시각을 포함하는 청크를 찾아 청크 내부에서 **비례 변환**한다:
      `media_start + (capture - start) × (media_dur / capture_dur)`.
      청크의 캡처 구간(`start_secs`~`end_secs`)은 카메라 시작·finalize 지연을 포함해
      실제 녹화분보다 넓다. 단순 오프셋은 청크 끝에서 `media_end`를 넘어 다음 청크
      구간으로 넘어간다.
    - 어떤 청크에도 속하지 않으면 `ok = false`. 사라진 52초(2.7%)는 청크 경계에서 촬영되지 않은
      시간인데, iOS는 캡처 구간이 연속이라 대부분 매핑되고(그 순간의 움직임만 영상에 없음),
      Android는 500ms 쿨다운으로 실제 공백이 생겨 `ok = false`가 발생한다.
